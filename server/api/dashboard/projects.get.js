...
export default defineEventHandler(async (event) => {
  ...
  let filteredProjects = db.analytics
    .filter(
      (project) =>
        project.name.toLowerCase().includes(queryLower) ||
        project.leader.toLowerCase().includes(queryLower) ||
        project.project.toLowerCase().includes(queryLower)
    )
    .reverse();
  if (sortByLocal) {
    ...
    if (sortByLocal === "leader") {
      filteredProjects = filteredProjects.sort((a, b) => {
        if (orderByLocal === "asc") return a.leader.localeCompare(b.leader);
        else return b.leader.localeCompare(a.leader);
      });
    }
